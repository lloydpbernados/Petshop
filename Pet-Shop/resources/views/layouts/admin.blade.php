...
<div class="flex min-h-screen relative">
    ...
    <aside :class="mobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'"
        ...
        <div class="p-6 xl:p-8">
            <div class="flex items-center justify-between">
                ...
                <button @click="mobileMenuOpen = false"
                        class="lg:hidden w-8 h-8 flex items-center justify-center rounded-xl bg-[#EBD7BC] text-[#2D241E] font-bold text-lg leading-none">✕</button>
            </div>
        ...
        @php
            ...
            $roleLabel  = ucfirst($authUser->role ?? 'User');
        @endphp
        <div class="p-6 xl:p-8 border-t border-[#EBD7BC]">
            <div class="flex items-center gap-3 xl:gap-4">
                ...
                <div class="overflow-hidden">
                    <p class="text-xs font-bold text-[#2D241E] truncate" title="{{ $authUser->name }}">
                        {{ $authUser->name }}
                    </p>
                    <p class="text-[10px] text-[#A68B6D] uppercase font-bold">{{ $roleLabel }}</p>
                </div>
            </div>
            ...
            <form method="POST" action="{{ route('logout') }}" class="mt-4">
                @csrf
                <button type="submit"
                        class="w-full text-left text-xs font-bold text-[#A68B6D] hover:text-[#E68A39] transition-colors flex items-center gap-2 px-1">
                    <span>🚪</span> Sign Out
                </button>
            </form>
        </div>
        ...
        <header class="h-16 sm:h-20 bg-white border-b border-[#F3E9DC] flex items-center justify-between px-4 sm:px-6 lg:px-10 sticky top-0 z-30">
            <div class="flex items-center gap-3 xl:gap-4">
                <button @click="mobileMenuOpen = true"
                        class="lg:hidden p-2 rounded-xl bg-[#FDF8F1] text-[#E68A39]">
            ...
            <div class="flex items-center gap-3">
                @auth
                    <span class="hidden sm:block text-xs font-semibold text-[#A68B6D]">
                        Welcome, {{ Auth::user()->name }}
                    </span>
                @endauth
                <span class="hidden sm:block text-[10px] font-bold text-[#A68B6D] uppercase tracking-widest">
                    Active Session
                </span>
