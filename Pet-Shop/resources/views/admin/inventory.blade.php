...
<div class="min-h-screen bg-[#FDF8F1]">
    <div class="px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-10">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-6 sm:mb-10 gap-4">
            <div>
                <div class="flex items-center gap-3 mb-2">
                    ...
                    <h1 class="font-serif-brand text-2xl sm:text-3xl lg:text-4xl font-bold text-[#2D241E] leading-tight">Pets Inventory</h1>
                </div>
            ...
            <div class="flex flex-col sm:flex-row gap-3 shrink-0 w-full sm:w-auto">
                <button class="w-full sm:w-auto bg-white border border-[#EBD7BC] text-[#5C4D3C] px-4 sm:px-6 py-3 rounded-xl sm:rounded-full font-bold shadow-sm hover:bg-[#FDF8F1] transition-all flex items-center justify-center gap-2 text-sm" onclick="exportCSV()">
                    <span>⬇</span> Export CSV
                </button>
                <button class="w-full sm:w-auto bg-[#E68A39] text-white px-5 sm:px-8 py-3 rounded-xl sm:rounded-full font-bold shadow-lg hover:bg-[#cf7529] transition-all flex items-center justify-center gap-2 text-sm" onclick="openAddModal()">
        ...
        {{-- Alert Banner --}}
        <div class="bg-[#FFF8E1] border border-[#FDE68A] rounded-xl sm:rounded-2xl p-3 sm:p-4 mb-6 sm:mb-8 flex items-start sm:items-center gap-3" id="alertBanner" style="display:none;">
            <span class="text-lg sm:text-xl shrink-0 mt-0.5 sm:mt-0">⚠️</span>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-6 sm:mb-10">
            <div class="bg-white p-4 sm:p-6 rounded-2xl sm:rounded-[2.5rem] border border-[#F3E9DC] shadow-sm flex items-center gap-4 sm:gap-5">
                <div class="p-3 sm:p-4 bg-[#E9F7F2] text-[#34A853] rounded-xl sm:rounded-2xl text-xl sm:text-2xl shrink-0">🏷️</div>
                <div class="min-w-0">
                    <p class="text-xs text-gray-400 font-bold uppercase tracking-widest leading-tight">Unique Breeds/Types</p>
                    <h3 class="text-xl sm:text-2xl font-bold text-[#2D241E] mt-1 truncate" id="statSku">—</h3>
                </div>
            </div>
            <div class="bg-white p-4 sm:p-6 rounded-2xl sm:rounded-[2.5rem] border border-[#F3E9DC] shadow-sm flex items-center gap-4 sm:gap-5">
                <div class="p-3 sm:p-4 bg-[#E9F0FE] text-[#4285F4] rounded-xl sm:rounded-2xl text-xl sm:text-2xl shrink-0">🐾</div>
                <div class="min-w-0">
                    <p class="text-xs text-gray-400 font-bold uppercase tracking-widest leading-tight">Total Live Pets</p>
                    <h3 class="text-xl sm:text-2xl font-bold text-[#2D241E] mt-1 truncate" id="statUnits">—</h3>
                </div>
            </div>
            <div class="bg-white p-4 sm:p-6 rounded-2xl sm:rounded-[2.5rem] border border-[#F3E9DC] shadow-sm flex items-center gap-4 sm:gap-5">
                <div class="p-3 sm:p-4 bg-[#FEF3C7] text-[#D97706] rounded-xl sm:rounded-2xl text-xl sm:text-2xl shrink-0">⚠️</div>
                <div class="min-w-0">
                    <p class="text-xs text-gray-400 font-bold uppercase tracking-widest leading-tight">Low / Unavailable</p>
                    <h3 class="text-xl sm:text-2xl font-bold text-[#2D241E] mt-1 truncate" id="statLow">—</h3>
                </div>
            </div>
            <div class="bg-white p-4 sm:p-6 rounded-2xl sm:rounded-[2.5rem] border border-[#F3E9DC] shadow-sm flex items-center gap-4 sm:gap-5">
                <div class="p-3 sm:p-4 bg-[#FDF2E9] text-[#E68A39] rounded-xl sm:rounded-2xl text-xl sm:text-2xl shrink-0">💰</div>
                <div class="min-w-0">
                    <p class="text-xs text-gray-400 font-bold uppercase tracking-widest leading-tight">Total Value</p>
                    <h3 class="text-xl sm:text-2xl font-bold text-[#2D241E] mt-1 truncate" id="statVal">—</h3>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-[1fr_300px] xl:grid-cols-[1fr_320px] gap-6 sm:gap-8 items-start">
            ...
            <div class="bg-white rounded-2xl sm:rounded-[2.5rem] border border-[#F3E9DC] shadow-sm overflow-hidden">
                <div class="p-4 sm:p-6 border-b border-[#FDF8F1] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <div class="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-3">
                        <div class="relative flex-1 min-w-0">
                            ...
                            <input type="text" id="searchQ" placeholder="Search pets…" oninput="loadInventory()"
                                   class="pl-9 pr-3 py-2.5 bg-[#FDF8F1] border border-[#F3E9DC] rounded-xl text-sm focus:outline-none focus:border-[#E68A39] w-full text-[#2D241E]">
                        </div>
                        <select id="filterCat" onchange="loadInventory()" class="w-full sm:w-auto px-3 py-2.5 bg-[#FDF8F1] border border-[#F3E9DC] rounded-xl text-sm focus:outline-none focus:border-[#E68A39] text-[#5C4D3C] cursor-pointer">
                        ...
                        <select id="filterStatus" onchange="loadInventory()" class="w-full sm:w-auto px-3 py-2.5 bg-[#FDF8F1] border border-[#F3E9DC] rounded-xl text-sm focus:outline-none focus:border-[#E68A39] text-[#5C4D3C] cursor-pointer">
                ...
                {{-- Mobile Cards --}}
                <div id="mobileCards" class="sm:hidden p-4 space-y-4"></div>
            ...
            <div class="flex flex-col gap-4 sm:gap-6">
                <div class="bg-white p-5 sm:p-6 rounded-2xl sm:rounded-[2.5rem] border border-[#F3E9DC] shadow-sm">
...
<div id="itemModal" class="fixed inset-0 bg-[#2D241E]/40 backdrop-blur-sm z-50 hidden items-center justify-center p-4">
    <div class="bg-white rounded-2xl sm:rounded-[2.5rem] p-5 sm:p-8 w-full max-w-[540px] shadow-2xl transform transition-all overflow-y-auto max-h-[90vh]">
        ...
        <div class="space-y-4 sm:space-y-5">
            ...
            <div>
                ...
                <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                    <div id="imagePreview" class="w-20 h-20 rounded-2xl bg-[#FDF8F1] border border-[#F3E9DC] flex items-center justify-center text-3xl shrink-0 overflow-hidden">
                        <span id="previewPlaceholder">📸</span>
                    </div>
                    <div class="flex-1 w-full">
                        <input id="mImage" type="file" accept="image/*" onchange="handleImageUpload(event)"
                               class="text-xs text-gray-500 file:mr-3 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-[#FDF2E9] file:text-[#E68A39] hover:file:bg-[#FCE1CC] cursor-pointer w-full"/>
            ...
            <div>
                ...
                <input id="mName" type="text" placeholder="e.g. Golden Retriever"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    {{-- onchange triggers SKU auto-generation --}}
                    <select id="mCat" onchange="generatePetSku()" class="w-full px-4 py-3 bg-[#FDF8F1] border border-[#F3E9DC] rounded-xl focus:outline-none focus:border-[#E68A39] text-[#2D241E] text-sm">
                ...
                <div>
                    ...
                    <select id="mGender" class="w-full px-4 py-3 bg-[#FDF8F1] border border-[#F3E9DC] rounded-xl focus:outline-none focus:border-[#E68A39] text-[#2D241E] text-sm">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input id="mSku" type="text" placeholder="Select category first"
                           class="w-full px-4 py-3 bg-[#FDF8F1] border border-[#F3E9DC] rounded-xl focus:outline-none focus:border-[#E68A39] text-[#2D241E] text-sm font-mono"/>
                </div>
                <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input id="mThresh" type="number" placeholder="2" min="1"
                           class="w-full px-4 py-3 bg-[#FDF8F1] border border-[#F3E9DC] rounded-xl focus:outline-none focus:border-[#E68A39] text-[#2D241E] text-sm"/>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea id="mDesc" rows="3" placeholder="Describe this pet breed, temperament, age, etc…"
        ...
        <div class="flex flex-col sm:flex-row gap-3 mt-6 sm:mt-8">
            <button onclick="closeModal('itemModal')" class="w-full sm:flex-1 px-4 sm:px-6 py-3 border border-[#EBD7BC] text-[#5C4D3C] rounded-xl font-bold hover:bg-[#FDF8F1] transition-colors text-sm">Cancel</button>
            <button onclick="saveItem()" id="saveBtn" class="w-full sm:flex-1 px-4 sm:px-6 py-3 bg-[#E68A39] text-white rounded-xl font-bold shadow-md hover:bg-[#cf7529] transition-colors text-sm">Save Pet</button>
        </div>
    </div>
</div>
...
<div id="restockModal" class="fixed inset-0 bg-[#2D241E]/40 backdrop-blur-sm z-50 hidden items-center justify-center p-4">
    <div class="bg-white rounded-2xl sm:rounded-[2.5rem] p-5 sm:p-8 w-full max-w-[400px] shadow-2xl">
        <div class="flex items-center justify-between mb-5 sm:mb-6">
        ...
        <div class="bg-[#FDF8F1] border border-[#F3E9DC] rounded-2xl p-4 flex items-center gap-4 mb-5 sm:mb-6">
            <div id="rIcon" class="w-12 h-12 bg-white rounded-xl flex items-center justify-center text-2xl shadow-sm overflow-hidden shrink-0"></div>
            <div class="min-w-0">
                <div id="rName" class="font-bold text-[#2D241E] text-sm sm:text-base truncate"></div>
                <div id="rCur" class="text-xs text-gray-500 mt-1 truncate"></div>
            </div>
        </div>
        <div class="space-y-4">
            <div>
            ...
            <div>
                ...
                <input id="rNote" type="text" placeholder="e.g. Local Certified Breeder"
                       class="w-full px-4 py-3 bg-[#FDF8F1] border border-[#F3E9DC] rounded-xl focus:outline-none focus:border-[#E68A39] text-[#2D241E] text-sm"/>
            </div>
        </div>
        <div class="flex flex-col sm:flex-row gap-3 mt-6 sm:mt-8">
            <button onclick="closeModal('restockModal')" class="w-full sm:flex-1 px-4 sm:px-6 py-3 border border-[#EBD7BC] text-[#5C4D3C] rounded-xl font-bold hover:bg-[#FDF8F1] transition-colors text-sm">Cancel</button>
            <button onclick="confirmRestock()" class="w-full sm:flex-1 px-4 sm:px-6 py-3 bg-[#34A853] text-white rounded-xl font-bold shadow-md hover:bg-[#2c8d46] transition-colors text-sm">Confirm Arrival</button>
        </div>
    </div>
</div>

<div id="toast" class="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 px-4 sm:px-6 py-3 sm:py-4 rounded-2xl shadow-xl font-bold text-xs sm:text-sm transform translate-y-10 opacity-0 pointer-events-none transition-all duration-300 flex items-center gap-3 max-w-[calc(100vw-2rem)]"></div>
...
<script>
    ...
    // Prefix map: category → 3-letter SKU prefix
    const PET_PREFIX = {Dogs:'DOG',Cats:'CAT',Birds:'BRD','Small Pets':'SMP',Fish:'FSH',Reptiles:'RPT'};
    ...
    // Counts how many existing pets in the loaded inventory already belong to
    // the selected category, then sets the next number in the series.
    // Format: DOG-001, DOG-002, CAT-001, etc.
    // Admin can still manually edit the field before saving.
    function generatePetSku() {
        if (editId) return; // don't overwrite SKU when editing an existing pet
        const cat = document.getElementById('mCat').value;
        if (!cat) return;
        const prefix = PET_PREFIX[cat] || cat.substring(0, 3).toUpperCase();
        // Count pets in this category from the currently loaded inventory
    ...
    async function loadInventory() {
        try {
            ...
            document.getElementById('mobileCards').innerHTML =
                '<div class="text-center py-12 text-red-400 text-sm bg-[#FDF8F1] border border-dashed border-[#F3E9DC] rounded-2xl">Failed to load pets.</div>';
        }
    }
    ...
    function renderTable() {
        ...
        inventory.forEach(item => {
            ...
            const catClass    = CAT_CLASS[item.category] || 'bg-gray-100 text-gray-600';
            ...
            <tr class="hover:bg-[#FDF8F1]/60 transition-colors group">
              ...
              <td class="px-4 sm:px-6 py-3 sm:py-4">
                <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase ${catClass}">${em} <span class="hidden sm:inline">${item.category}</span></span>
              </td>
              ...
              <td class="px-4 sm:px-6 py-3 sm:py-4">
                <div class="flex items-center gap-1 bg-[#FDF8F1] border border-[#F3E9DC] rounded-xl p-1 w-fit">
                  <button class="w-6 h-6 flex items-center justify-center rounded-lg text-[#991B1B] hover:bg-[#FEE2E2] font-bold text-sm" onclick="adjustStock(${item.id},-1)">−</button>
                  <span class="text-sm font-bold text-[#2D241E] w-8 text-center" id="qty-${item.id}">${item.stock}</span>
                  <button class="w-6 h-6 flex items-center justify-center rounded-lg text-[#166534] hover:bg-[#E9F7F2] font-bold text-sm" onclick="adjustStock(${item.id},+1)">+</button>
                </div>
              </td>
              <td class="px-4 sm:px-6 py-3 sm:py-4">
              ...
              <td class="px-4 sm:px-6 py-3 sm:py-4 text-right whitespace-nowrap">
                <div class="flex items-center justify-end gap-2">
                  <button class="px-3 py-1.5 text-xs font-bold rounded-xl bg-[#F3E9DC]/50 text-gray-600 hover:bg-[#EBD7BC]" onclick="openEditModal(${item.id})">Edit</button>
                  <button class="px-3 py-1.5 text-xs font-bold rounded-xl bg-[#FDF2E9] text-[#E68A39] hover:bg-[#FCE1CC]" onclick="openRestockModal(${item.id})">Arrival</button>
                  <button class="px-3 py-1.5 text-xs font-bold rounded-xl bg-red-50 text-red-500 hover:bg-red-100" onclick="deleteItem(${item.id})">Delete</button>
                </div>
              </td>
            </tr>`;
            ...
            <div class="bg-white border border-[#F3E9DC] rounded-2xl p-4 shadow-sm">
                <div class="flex items-start justify-between mb-3">
                    <div class="flex items-center gap-3">
                        <div class="w-12 h-12 rounded-xl bg-[#FDF8F1] border border-[#F3E9DC] flex items-center justify-center shrink-0 overflow-hidden">${displayImage}</div>
                        <div class="min-w-0">
                            <div class="font-bold text-[#2D241E] text-sm flex items-center truncate">${item.name}${genderIcon}</div>
                            <div class="text-[10px] text-gray-400 font-mono mt-0.5 truncate">${item.sku ?? 'No SKU'}</div>
                            ${item.description ? `<div class="text-[10px] text-gray-400 mt-0.5 truncate max-w-[180px]">${item.description}</div>` : ''}
                        </div>
                    </div>
                    ${statusBadge}
                </div>
                <div class="grid grid-cols-2 gap-x-4 gap-y-3 mb-4 text-xs">
                    <div>
                        <span class="text-gray-400 block text-[10px] uppercase font-bold tracking-wider mb-0.5">Category</span>
                        <span class="font-medium text-[#2D241E] inline-flex items-center gap-1">${em} ${item.category}</span>
                    </div>
                    <div>
                        <span class="text-gray-400 block text-[10px] uppercase font-bold tracking-wider mb-0.5">Headcount</span>
                        <span class="font-bold text-[#2D241E]">${item.stock}</span>
                    </div>
                    <div>
                        <span class="text-gray-400 block text-[10px] uppercase font-bold tracking-wider mb-0.5">Price</span>
                        <span class="font-medium text-[#2D241E]">₱${parseFloat(item.price).toLocaleString('en-PH',{minimumFractionDigits:2})}</span>
                    </div>
                    <div>
                        <span class="text-gray-400 block text-[10px] uppercase font-bold tracking-wider mb-0.5">Total Value</span>
                        <span class="font-medium text-[#2D241E]">₱${val}</span>
                    </div>
                </div>
                <div class="flex flex-col sm:flex-row items-center justify-between pt-3 border-t border-[#F3E9DC] gap-3">
                    <div class="flex items-center bg-[#FDF8F1] border border-[#F3E9DC] rounded-xl p-1 w-full sm:w-auto justify-center">
                        <button class="w-10 h-10 flex items-center justify-center rounded-lg text-[#991B1B] hover:bg-[#FEE2E2] font-bold text-xl" onclick="adjustStock(${item.id},-1)">−</button>
                        <span class="text-sm font-bold text-[#2D241E] w-10 text-center">${item.stock}</span>
                        <button class="w-10 h-10 flex items-center justify-center rounded-lg text-[#166534] hover:bg-[#E9F7F2] font-bold text-xl" onclick="adjustStock(${item.id},+1)">+</button>
                    </div>
                    <div class="flex gap-2 w-full sm:w-auto justify-center">
                        <button class="flex-1 sm:flex-none px-3 py-2 text-xs font-bold rounded-xl bg-white border border-[#F3E9DC] text-gray-600 hover:bg-gray-50" onclick="openEditModal(${item.id})">Edit</button>
                        <button class="flex-1 sm:flex-none px-3 py-2 text-xs font-bold rounded-xl bg-[#FDF2E9] text-[#E68A39] hover:bg-[#FCE1CC]" onclick="openRestockModal(${item.id})">Arrival</button>
                        <button class="flex-1 sm:flex-none px-3 py-2 text-xs font-bold rounded-xl bg-red-50 text-red-500 hover:bg-red-100" onclick="deleteItem(${item.id})">Delete</button>
                    </div>
                </div>
            </div>`;
        });

        const emptyState = `<div class="text-center py-12 text-gray-400 text-sm bg-[#FDF8F1] border border-dashed border-[#F3E9DC] rounded-2xl"><div class="text-4xl mb-3 opacity-50">📭</div><p>No pets match your filters.</p></div>`;
        tbody.innerHTML           = rows       || `<tr><td colspan="6" class="text-center py-12 text-gray-400 text-sm">No pets found.</td></tr>`;
        mobileContainer.innerHTML = mobileCards || emptyState;
        updateStats(); updateBreakdown(); updateRestockList();
    }
    ...
    function updateRestockList() {
        ...
        if (!need.length) { document.getElementById('restockList').innerHTML = `<p class="text-sm text-gray-400 text-center py-4 bg-[#FDF8F1] rounded-2xl border border-[#F3E9DC]">✅ All pets are well-supplied!</p>`; return; }
        document.getElementById('restockList').innerHTML = need.map(item => `
            <div class="flex items-center justify-between py-3 border-b border-[#FDF8F1] last:border-0">
                <div class="flex items-center gap-2 sm:gap-3 min-w-0">
                    <div class="w-8 h-8 sm:w-10 sm:h-10 rounded-lg bg-[#FDF8F1] border border-[#F3E9DC] overflow-hidden flex items-center justify-center text-xs shrink-0">
                        ${item.image ? `<img src="${item.image}" class="w-full h-full object-cover">` : (CAT_EMOJI[item.category]??'🐾')}
                    </div>
                    <div class="min-w-0">
                        <div class="text-xs sm:text-sm font-bold text-[#2D241E] truncate">${item.name}</div>
                        <div class="text-[10px] ${item.status==='out'?'text-red-500':'text-[#E68A39]'} font-bold mt-0.5 uppercase">${item.status==='out'?'Unavailable':'Low Stock'}</div>
                    </div>
                </div>
                <button class="px-3 py-2 rounded-xl bg-[#FDF2E9] text-[#E68A39] text-xs font-bold hover:bg-[#FCE1CC] shrink-0" onclick="openRestockModal(${item.id})">Arrival</button>
            </div>`).join('');
    }

    async function adjustStock(id, delta) {
        try {
            const res  = await fetch(`${API}/pets/${id}/stock`, { method:'PATCH', headers:{'Content-Type':'application/json','X-CSRF-TOKEN':CSRF}, body:JSON.stringify({delta}) });
    ...
    function openAddModal() {
        ...
        ['mName','mSku','mStock','mPrice','mDesc'].forEach(id => document.getElementById(id).value = '');
    ...
    async function saveItem() {
        ...
        try {
            ...
            const res = await fetch(url, { method:'POST', headers:{'X-CSRF-TOKEN':CSRF}, body:form });
    ...
    async function confirmRestock() {
        ...
        try {
            const res  = await fetch(`${API}/pets/${restockId}/restock`, { method:'POST', headers:{'Content-Type':'application/json','X-CSRF-TOKEN':CSRF}, body:JSON.stringify({qty}) });
            const data = await res.json();
    ...
    async function deleteItem(id) {
        ...
        try {
            await fetch(`${API}/pets/${id}`, { method:'DELETE', headers:{'X-CSRF-TOKEN':CSRF} });
    ...
    function showToast(msg, type = 'success') {
        ...
        t.innerHTML = `<span>${type==='success'?'✅':'⚠️'}</span> <span class="truncate">${msg}</span>`;
        t.className = `fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 ${type==='success'?'bg-[#34A853]':'bg-[#EF4444]'} text-white px-4 sm:px-6 py-3 sm:py-4 rounded-2xl shadow-xl font-bold text-xs sm:text-sm transform transition-all duration-300 flex items-center gap-3 max-w-[calc(100vw-2rem)]`;
    ...
    function exportCSV() {
        const header = 'Breed/Type,SKU,Category,Gender,Headcount,Price,Description\n';
        const rows   = inventory.map(i => `"${i.name}","${i.sku??''}","${i.category}","${i.gender??'N/A'}",${i.stock},${i.price},"${i.description??''}"`).join('\n');
